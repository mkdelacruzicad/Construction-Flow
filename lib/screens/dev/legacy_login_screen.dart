...
class _LoginScreenState extends State<LoginScreen> {
              ...
              child: Column(crossAxisAlignment: CrossAxisAlignment.stretch, children: [
                ...
                Form(
                  ...
                  child: Column(children: [
                    TextFormField(controller: _emailCtrl, decoration: const InputDecoration(labelText: 'Work email', prefixIcon: Icon(Icons.mail)), keyboardType: TextInputType.emailAddress, autofillHints: const [AutofillHints.username, AutofillHints.email], validator: (v) { if (v == null || v.isEmpty) return 'Email is required'; if (!v.contains('@')) return 'Enter a valid email'; return null; }),
                    const SizedBox(height: 16),
                    TextFormField(controller: _passwordCtrl, decoration: InputDecoration(labelText: 'Password', prefixIcon: const Icon(Icons.lock), suffixIcon: IconButton(onPressed: () => setState(() => _obscure = !_obscure), icon: Icon(_obscure ? Icons.visibility : Icons.visibility_off, color: cs.onSurfaceVariant))), obscureText: _obscure, validator: (v) => (v == null || v.isEmpty) ? 'Password is required' : null),
                  ]),
                ),
                const SizedBox(height: 24),
                SizedBox(
  ...
  Future<void> _onSignIn() async {
    ...
    try {
      final ok = context.read<MockDataService>().loginWithEmail(email: _emailCtrl.text.trim(), password: _passwordCtrl.text, role: role);
      if (!ok) { _showError('Incorrect email or password for this role.'); return; }

      switch (role) {
        case UserRole.procurement:
          if (!mounted) return; context.go('/procurement-shell/dashboard');
          break;
        case UserRole.supplier:
          if (!mounted) return; context.go('/supplier-shell/dashboard');
          break;
        case UserRole.warehouse:
          if (!mounted) return; context.go('/warehouse-shell/dashboard');
          break;
      }
    } catch (e) {
